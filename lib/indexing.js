...
class QdrantBatchProcessor {
  ...
  async processAllVTTFiles(vttDirectory) {
    ...
    for (let i = 0; i < fileBatches.length; i++) {
      ...
      try {
        ...
        console.log(`📄 Loaded ${batchDocs.length} documents from ${fileBatch.length} files`);
        ...
        console.error(`❌ Failed to process file batch ${i + 1}:`, error.message);
        throw error;
      }
    }
  ...
  async processDocumentBatches(documents, isInitialized) {
    const documentBatches = this.createBatches(documents, this.documentBatchSize);
    ...
    for (let j = 0; j < documentBatches.length; j++) {
      ...
      console.log(`    📦 Document batch ${j + 1}/${documentBatches.length} (${docBatch.length} docs)`);
      ...
      try {
        if (!isInitialized && j === 0) {
          ...
          console.log(`    🆕 Initialized Qdrant collection with ${docBatch.length} documents`);
        } else {
          // Add documents to existing collection
          await this.addDocumentsToExistingStore(docBatch);
          console.log(`    ➕ Added ${docBatch.length} documents to collection`);
        }
        ...
        if (error.message.includes('Payload error') || error.message.includes('larger than allowed')) {
  ...
  async retryWithSmallerBatch(documents, isInitialized) {
    ...
    for (let k = 0; k < smallerBatches.length; k++) {
      ...
      console.log(`      🔹 Retry batch ${k + 1}/${smallerBatches.length} (${tinyBatch.length} docs)`);
